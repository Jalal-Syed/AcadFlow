...
export interface PortalCapturePlugin {
    ...
    close(): Promise<void>;
    addListener(eventName: 'capture', listenerFunc: (payload: PortalCapturePayload) => void): Promise<PluginListenerHandle>;
    addListener(eventName: 'closed', listenerFunc: () => void): Promise<PluginListenerHandle>;
    addListener(eventName: 'error', listenerFunc: (payload: {
        message: string;
